...
// New initialize mini theme
func New(config *auth.Config) *auth.Auth {
	if config == nil {
		config = &auth.Config{}
	}
	config.ViewPaths = append(config.ViewPaths, "github.com/naokij/auth_themes/mini/views")

	if config.DB == nil {
		fmt.Print("Please configure *gorm.DB for Auth theme mini")
	}
	...
	if config.Render == nil {
		...
		for _, gopath := range append([]string{filepath.Join(utils.AppRoot, "vendor")}, utils.GOPATH()...) {
			filePath := filepath.Join(gopath, "src", "github.com/naokij/auth_themes/mini/locales/en-US.yml")
