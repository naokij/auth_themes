...
func New(config *auth.Config) *auth.Auth {
	...
	config.ViewPaths = append(config.ViewPaths, "github.com/qor/auth_themes/mini/views")
	...
	if config.Render == nil {
		...
		for _, gopath := range append([]string{filepath.Join(utils.AppRoot, "vendor")}, utils.GOPATH()...) {
			filePath := filepath.Join(gopath, "src", "github.com/qor/auth_themes/mini/locales/en-US.yml")
